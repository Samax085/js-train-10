...
function customAssign(...objects) {
  ...
  let arrObj = Object.assign({}, ...objects);  
  ...
  for (let key in arrObj) {
    ...
    if (typeof arrObj[key] === "number") {
      arrObj[key] = arrObj[key].toString()
    }
  }  
...
function customObjectFromEntries(entries) {
  ...
  let mapEntries = entries.map(([key, value]) => {
...
const objEntries = Object.fromEntries(mapEntries)
...
function checkProperty(obj, prop) {
  ...
  const newObj = obj.hasOwnProperty(prop)
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  let addProp = Object.assign(obj1, obj2)
...
function filterObjectsByKey(arr, key) {
  ...
  const arrFilter = arr.filter((item) => item.hasOwnProperty(key))
